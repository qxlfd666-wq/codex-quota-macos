...
1. 下载最新版 `Codex-Quota-Windows-x64.exe`，放到希望长期保存的位置。
2. 双击运行，然后打开 Codex。
...
v1.0.1 已修复部分安装中误选 Codex 图形界面 helper 的问题。如果托盘提示“无法获取额度”：

1. 升级到 v1.0.1 或更高版本。
2. 打开官方 Codex 客户端，并确认已使用 ChatGPT 账户登录；API Key 按量计费模式不会返回套餐百分比。
3. 右键托盘的 `%` 图标，选择“复制诊断信息”，反馈问题时附上该信息。无需也请勿复制登录 token 或 `auth.json` 内容。

如果自动检测仍失败，高级用户可将环境变量 `CODEX_QUOTA_CODEX_PATH` 设为能运行 `app-server` 的 `codex.exe` 完整路径，然后重启 Codex Quota。
